            ...
            unless count == index+1
              loop do
                (i..count-1).each_with_index do |ib, ind|
                  unless @bus_detail.bus_routes[count-j].name == br.name
                    bd = BusDetail.create(name: @bus_detail.name, number: @bus_detail.number, source: @bus_detail.bus_routes[count-j].name, destination: br.name)
                    bd.get_geo_location
                  end
                  unless (count-2) <= i
